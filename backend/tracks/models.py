...
class Track(models.Model):
    direction = models.ForeignKey(
        Direction, on_delete=models.CASCADE, related_name="tracks"
    )
    ...
    duration = models.CharField(
        max_length=100, blank=True, verbose_name="Длительность обучения"
    )
...
class Checklist(models.Model):
    track = models.ForeignKey(
        Track, on_delete=models.CASCADE, related_name="checklists"
    )
...
class Task(models.Model):
    ...
    points = models.IntegerField(
        default=1, validators=[MinValueValidator(1)], verbose_name="Баллы"
    )
    order = models.IntegerField(default=0, verbose_name="Порядок")
    is_auto_check = models.BooleanField(default=True)
...
class UserTaskAttempt(models.Model):
    user = models.ForeignKey(
        "accounts.User", on_delete=models.CASCADE, related_name="task_attempts"
    )
...
class PracticalSubmission(models.Model):
    ...
    user = models.ForeignKey(
        "accounts.User", on_delete=models.CASCADE, related_name="practical_submissions"
    )
