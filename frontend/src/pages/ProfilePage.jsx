...
export const ProfilePage = () => {
  ...
  const [progress, setProgress] = useState([]); // Инициализируем как пустой массив
  ...
  const loadProgress = async () => {
    try {
      const data = await trackService.getUserProgress();

      // Если data — это массив, используем его. Если нет, пытаемся извлечь из data.results
      let progressData = [];
      if (Array.isArray(data)) {
        ...
        // Если data — объект, но не массив, возможно, это один элемент прогресса
        progressData = [data];
      }
    ...
    <Container className="py-8">
      <Paper className="p-6 mb-6">
        <div className="flex items-center gap-4 mb-6">
          <Avatar src={user.avatar || undefined} sx={{ width: 80, height: 80, bgcolor: '#37EBFF', color: '#0A1E64' }}>
        ...
        <Button variant="contained" onClick={handleUpdate} className="mt-6" sx={{ background: 'linear-gradient(135deg, #37EBFF 0%, #0541F0 100%)' }}>
          Сохранить изменения
        </Button>
      </Paper>
      <Paper className="p-6">
        ...
        {loading ? (
          ...
          <Typography color="textSecondary">Вы ещё не начали ни один трек.</Typography>
        ) : (
          progress.map((p) => (
            <div key={p.id} className="mb-3">
              <Typography>{p.track_name}</Typography>
              <div className="progress-bar">
                <div className="progress-bar-fill" style={{ width: `${(p.completed_tasks_count / p.total_tasks_count) * 100}%` }} />
              </div>
              <Typography variant="caption">{p.completed_tasks_count} / {p.total_tasks_count} заданий</Typography>
            </div>
          ))
        )}
      </Paper>
