  ...
  return (
    <>
      <Box className="relative overflow-hidden bg-gradient-to-br from-darkBlue via-blue to-cyan text-white">
        <Container className="relative py-20 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <Typography variant="h2" component="h1" gutterBottom fontWeight="800" className="text-white">Выбери свой путь в IT</Typography>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}>
              <Link to="/tracks" className="inline-block px-8 py-3 rounded-xl bg-white text-blue font-bold text-lg hover:shadow-xl transition hover:scale-105">Начать выбор</Link>
            </motion.div>
          </motion.div>
        </Container>
      ...
      <Container className="py-20">
        <Typography variant="h3" component="h2" textAlign="center" gutterBottom fontWeight="700" className="text-darkBlue">Почему стоит выбрать нас?</Typography>
        <Typography variant="h6" textAlign="center" className="text-darkGray mb-12">Всё, что нужно для осознанного выбора будущей профессии</Typography>
          ...
          {features.map((f, idx) => (
            <Grid item xs={12} md={4} key={idx}>
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }} className="text-center group">
                <div className="inline-block p-4 rounded-2xl bg-gradient-to-r from-cyan/10 to-blue/10 mb-4 group-hover:scale-110 transition">{f.icon}</div>
